...
const PriceCard = React.forwardRef(
  ({ title, price, description, features, buttonText, isSelected, onSelect }, ref) => (
    <div
      ref={ref}
      onClick={onSelect}
      className={`cursor-pointer p-5 sm:p-6 md:p-8 rounded-[32px] border transition-all duration-500 flex flex-col
      w-[85%] xs:w-[75%] sm:w-full min-w-[240px] sm:min-w-0
      snap-center
      ${isSelected
          ? 'bg-[#5e5df0] text-white border-[#5e5df0] shadow-[0_20px_50px_rgba(94,93,240,0.3)] scale-[1.02] sm:scale-[1.05] z-10'
          : 'bg-white text-gray-900 border-gray-100 shadow-sm hover:shadow-xl scale-100 opacity-90 sm:opacity-100'
        }`}
    >
      <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
